...
class STARFusionIO(HashedSVIO):
    ...
    def __init__(self, filepath, mode="r", annot_field="FCANN", sample_name="sample", write_without_est=False):
        """
        ...
        :param sample_name: The name of the sample.
        :type sample_name: str
        :type write_without_est: This option is only used when writing to a non-existing file. If True, output is that of STAR-Fusion before version 1.9 (without est_J and est_S).
        :param write_without_est: bool
        :return: The new instance.
        :rtype: STARFusionIO
        """
        ...
        self.annot_field = annot_field
        self.sample_name = sample_name
        if self.titles is None:  # Writing to a non-existing file
            if self.write_without_est:
                self.titles = [elt for elt in STARFusionIO.titles if elt not in {"est_J", "est_S"}]
            else:
    ...
    def _dictToBNDRecords(self, fusion_record):
        ...
        est_J, est_S = None, None
        if "est_J" in fusion_record:
            est_J = float(fusion_record["est_J"])
            est_S = float(fusion_record["est_S"])
        FFPM = float(fusion_record["FFPM"])
        large_anchor_support = "1" if fusion_record["LargeAnchorSupport"] == "YES_LDAS" else "0"
        info_annot_tags = []
        for section in fusion_record["annots"].split(";"):  # Example: ["RUNX1:ArcherDX_panel","RUNX1:FoundationOne_panel","RUNX1:Oncogene"];["RUNX1T1:Oncogene","RUNX1T1:ArcherDX_panel","RUNX1T1:FoundationOne_panel"];["Mitelman","ChimerKB","DepMap2023","ChimerSeq","TCGA_StarF2019","CCLE_StarF2019","YOSHIHARA_TCGA","Klijn_CellLines","chimerdb_pubmed","chimerdb_omim","DEEPEST2019","GUO2018CR_TCGA","TumorFusionsNAR2018"];INTERCHROMOSOMAL[chr21--chr8]
            if section.startswith("[") or section.startswith('"'):
                info_annot_tags += [elt.strip() for elt in json.loads(section)]
            else:
                info_annot_tags.append(section)
        ...
        for side, mate_side in [("Left", "Right"), ("Right", "Left")]:
            ...
            samples_field = {
                self.sample_name: {
                    "PR": PR, "SR": SR, "estJ": est_J, "estS": est_S,
                    "hasLAS": large_anchor_support, "FFPM": FFPM
                }
            }
            if side == "Left":
                info["RNA_FIRST"] = True
    ...
    def _BNDRecordsToDict(self, breakends):
        ...
        return {
            ...
            "est_J": first.samples[self.sample_name]["estJ"] if "estJ" in first.samples[self.sample_name] else None,
            "est_S": first.samples[self.sample_name]["estS"] if "estS" in first.samples[self.sample_name] else None,
    ...
    def isValid(filepath):
        ...
        try:
            with SVIO(filepath, title_starter="#") as reader:
                mandatory_fields = set(STARFusionIO.titles) - {"JunctionReads", "SpanningFrags", "est_J", "est_S"}
    ...
    def setVCFHeader(vcf_io, annotation_field="FCANN"):
        ...
        vcf_io.format = {
            ...
            "estJ": HeaderFormatAttr("estJ", type="Float", number="1", description="Number of RNA-Seq fragments that encompass the fusion junction such that one read of the pair aligns to a different gene than the other paired-end read of that fragment after assigning reads to alt splice fusion isoforms (est_J)."),
            "estS": HeaderFormatAttr("estS", type="Float", number="1", description="Number of RNA-Seq fragments containing a read that aligns as a split read at the site of the putative fusion junction after assigning reads to alt splice fusion isoforms (est_S)."),
